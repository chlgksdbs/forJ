    ...
    <div id="board_table_box">
      <table id="board_write_table">
        <tr class="board_table_line">
          <td><b>제목</b></td>
          <td><input type="text" id="title" v-model="noticeTitle" maxlength="45" placeholder="제목을 입력해 주세요." /></td>
        </tr>
        <tr class="board_table_line">
          <td><b>작성자</b></td>
          <td>{{ noticeWriter }}</td>
...
export default {
  ...
  created() {
    ...
    this.noticeWriter = this.userInfo.userId;
  },
  computed: {
  ...
  methods: {
    writeBtn() {
      ...
      this.noticeContent = this.$refs.toastuiEditor.invoke("getMarkdown");
