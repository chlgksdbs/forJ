    ...
    <div id="board_table_box">
      <table id="board_write_table">
        <tr class="board_table_line">
          <td><b>제목</b></td>
          <td><input type="text" id="title" maxlength="45" v-model="boardView.title" placeholder="제목을 입력해 주세요." /></td>
        </tr>
        <tr class="board_table_line">
          <td><b>작성자</b></td>
        ...
        <tr class="board_table_line">
          ...
          <td>
            <div class="board_content_box">
              <textarea :value="boardView.content">
              </textarea>
            </div>
          </td>
        </tr>
...
export default {
  ...
  created() {
    axios.get('http://localhost/board/detail/' + this.$route.params.boardId)
      .then((resp) => {
        this.boardView = resp.data;
        // console.log(resp.data); // 디버깅 
      });
  },
  mounted() {
    this.$refs.toastuiEditor.invoke("setMarkdown", this.boardView.content);
  },
  computed: {
  ...
  methods: {
    ...
    modifyBtn() {
      ...
      this.boardView.content = this.$refs.toastuiEditor.invoke("getMarkdown");

      // content를 html 형태로 저장하는 action
      // var content = this.$refs.toastuiEditor.invoke("getHtml");

      axios.put('http://localhost/board/modify', this.boardView)
        .then(() => {
